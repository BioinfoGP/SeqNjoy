---
title: Quality report for `r qcData['File Name']` and `r qcData2['File Name']`
---
...
`r knitr::kable(summaryData, align="lrr")`

```{r, sample-size-warning, eval=showSampleSizeWarning,echo=FALSE}
	if(showSampleSizeWarning){
		knitr::asis_output(paste0("# <span style='color:red'> WARNING! FASTQ file integrity issue detected! </span> \n\n ## Sample size reported by Rqc (",rqcSampleReads,") differs from fastp (",totalSampleReads,").\n\n ## This suggests potential issues with your FASTQ file(s). Do not proceed with filtering until the file(s) are fixed. "))
	}
```
...
```{r, echo=FALSE,out.width="100%", fig.width=10,fig.height=5, fig.cap=""}
# Quality per cycle box plot 
boxplotData<-rqcData2$CycleQualityHist 
boxplotData$cycle<-as.numeric(boxplotData$cycle)
maxQuality<-max(DEFAULT_MAXQUALITY,max(boxplotData$ymax))
plotData_even <- subset(boxplotData, cycle %% 2 == 0)
plotData_odd <- subset(boxplotData, cycle %% 2 == 1)
p<-ggplot(boxplotData) +
  ...
  geom_segment(aes(x = cycle - 0.3, xend = cycle + 0.3, y = middle, yend = middle), 
               color = "red", linewidth = 1) +
  # Draw the line across average
  geom_line(data=rqcData2$ReadQuality,aes(x = as.numeric(cycle), y = quality), color = "blue", linewidth = 0.5) +  
  # Add labels and theme
  labs(x = "Cycle", y = "Quality", title = "Per cycle Quality") +
  theme_classic()+
#  theme(axis.text.x = element_text(angle = -45))+
...
p<-ggplot(data=combinedDf) +
  ...
  labs(x = "Cycle") +
  scale_color_manual(name="",breaks = c("A","C","G","T","N"), labels=c("%A","%C","%G","%T","%N"), values=c(A="red",C="blue",G="green",T="black",N="purple"))+
  theme_classic()+
...
```{r, echo=FALSE,out.width="100%", fig.width=10,fig.height=5, fig.cap=""}
# Nucleotide content per cycle line plot  (before filtering)
ntContent <- qcData2$NucleotideContent %>% select(-c("GC"))
ntContent$POSITION=seq_along(ntContent$A)
meltDf<-reshape2::melt(ntContent,id.vars="POSITION", variable.name = "NT", value.name = "PERCENT")

if(showFiltered){
	# Nucleotide content per cycle line plot  (after filtering)
	filteredNtContent <- qcData2$FilteredNucleotideContent %>% select(-c("GC"))
	filteredNtContent$POSITION=seq_along(filteredNtContent$A)
	filteredMeltDf<-reshape2::melt(filteredNtContent,id.vars="POSITION", variable.name = "NT", value.name = "PERCENT")
...
duplicationNames<-c(as.character(1:9),">10",">50",">100",">500",">1k",">5k",">10k")
duplicationNames<-factor(duplicationNames,levels = unique(duplicationNames))
plotData<-rqcData2$DuplicationFrequencies
dupRateData<-plotData %>%
  mutate(INTERVAL = cut(occurrence, breaks = duplicationBreaks, right = FALSE)) %>%
  group_by(INTERVAL) %>%
  summarise(TOTAL = sum(percentage),DEDUPLICATED = sum(deduplicated))
...
p<-ggplot(data=meltDf,aes(x=INTERVAL,y=PERCENTAGE,color=SEQUENCES, group=SEQUENCES)) +
  ...
  geom_tile(data=oddColumns, aes(x = INTERVAL, y = 50, width=1, height=100),fill = rgb(0.7, 0.7, 0.7, alpha = 0.5), inherit.aes = FALSE)+  
  geom_line(linewidth=1)+    
  labs(title = paste0("Percent of sequences remaining if deduplicated: ",rqcData2$DuplicationRate ,"%")) +
  scale_x_discrete(name="Number of repetitions", breaks=levels(meltDf$INTERVAL), labels=duplicationNames,expand = c(0, 0.5))+
  scale_y_continuous(name="Percentage", breaks=seq(0,100,10), labels=seq(0,100,10),sec.axis = dup_axis(name = NULL, labels = NULL),expand = c(0, 0))  +
  scale_color_manual(name="", breaks=levels(meltDf$SEQUENCES), labels=levels(meltDf$SEQUENCES), values=c(TOTAL="blue",DEDUPLICATED="red"))  +
  guides(colour = guide_legend(position = "inside"))+
...
`r if(overrepresented1) knitr::kable(overrepresentedDataSummary, align="lrr")  else "No overrepresented sequences found in read 1"`

## Read 2

`r if(overrepresented2) knitr::kable(overrepresentedDataSummary2, align="lrr")  else "No overrepresented sequences found in read 2"`
...
```{r,eval=adapter2, echo=FALSE, out.width="100%", fig.width=10,fig.height=5, fig.cap=""}
# Cumulative percentage of occurrences of the adapter at each position: 
if(adapter2){
	plotData<-data.frame(POSITION=seq_along(adapter_cumsum2),CUMSUM=adapter_cumsum2)
	rectData<-subset(plotData, POSITION %% 2 == 1)
	maxY<-100
	p<-ggplot(data=plotData) +
	  geom_rect(data=rectData, aes(xmin = POSITION - 0.5, xmax = POSITION + 0.5, ymin = 0,ymax = maxY), fill = rgb(0.7, 0.7, 0.7, alpha = 0.5))+
...
`r if(adapter2) knitr::kable(AdapterStats2, align="lr") else "Adapter not found"`


```{r, insersize-heading, eval=insertSizeDetected,echo=FALSE}
	if(insertSizeDetected){
		knitr::asis_output("# Insert size estimation")
	}
```


```{r,eval=insertSizeDetected, echo=FALSE, out.width="100%", fig.width=10,fig.height=5, fig.cap=""}
if(insertSizeDetected){
	rectData<-data.frame(FROM=seq(0, max(insertSizeData$Size), by = 5))
	rectData$TO<-pmin(rectData$FROM + 5, max(insertSizeData$Size))    
	rectData<-rectData[seq(1, nrow(rectData), by = 2), ]

	p<-ggplot(insertSizeData, aes(x=Size, y=Frequency)) +
	  geom_rect(data=rectData, aes(xmin = FROM, xmax = TO, ymin = 0,ymax = max(insertSizeData$Frequency)), fill = rgb(0.7, 0.7, 0.7, alpha = 0.5), inherit.aes = FALSE)+ 
	  geom_line(color="blue")+
	  geom_vline(xintercept=insertSizePeak, color = "red", linetype = "dashed", linewidth = 1)+  
	  theme_classic()+
	  scale_y_continuous(name="Read Percent", sec.axis = dup_axis(name = NULL, labels = NULL), expand = c(0, 0))+  
	  scale_x_continuous(name="Insert size", breaks=c(lineTicks), labels=c(lineTicks), sec.axis = dup_axis(name = NULL, labels = NULL),expand = c(0, 0))+   
	  labs(title = paste0("Insert size distribution (",unknownFreq,"%  reads are of unknown length)")) + 
	  theme(axis.line.x=element_line(linewidth = 0.5, colour = "lightgrey"),
		axis.line.y.right=element_line(linewidth = 0.5, colour = "lightgrey"),
		axis.ticks.y.right = element_line(linewidth = 0.5, colour = "lightgrey"),
		panel.grid.major.y = element_line(linewidth = 0.5, colour = "lightgrey"),
		panel.grid.major.x = element_line(linewidth = 0.5, colour = "lightgrey"))+
	  geom_text(data=as.data.frame(insertSizePeak), aes(x = insertSizePeak, y = insertSizeData[insertSizePeak,]$Frequency/2, label = insertSizePeak), color = "red", vjust = 0.5, hjust = -0.1,size = 4)
...
```{r, average-quality-heading1, eval=showFiltered,echo=FALSE}
	if(showFiltered){
		knitr::asis_output("## Read 1")
	}
```	
...
```{r, average-quality-heading2, eval=showFiltered,echo=FALSE}
	if(showFiltered){
		knitr::asis_output("## Read 2")
	}
```	
...
if(showFiltered){
	# Per cycle Quality (before and after filtering)
	meltDf<-reshape2::melt(qcData2$Quality,id.vars="POSITION", variable.name = "GROUP", value.name = "QUALITY")
	maxQuality<-max(DEFAULT_MAXQUALITY,max(meltDf$QUALITY))
	plotData_even <- subset(meltDf, POSITION %% 2 == 0)
	plotData_odd <- subset(meltDf, POSITION %% 2 == 1)
	p<-ggplot(data=meltDf, aes(x=POSITION,y=QUALITY, linetype=GROUP)) +
