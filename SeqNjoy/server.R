...
#	finnyDEBUG <<- 2
	...
	setFunctionParams<-function(file) {
		...
		return(read.table(text=cleaned_content, sep="\t", header=TRUE, quote="", comment.char="#"))
	}
...
#	myDfSessions<-reactiveValues(sessions=data.frame(id=numeric(), status=character(), started=character(), accessed=character(), datapath=character(), description=character(),stringsAsFactors=F))
	myDfSessions <<- data.frame(
		id=numeric()
		,status=character()
		,started=character()
		,accessed=character()
	...
	#home_dir <<- paste0(finny_dir,'/www/PROJECTS')
	home_dir <<- "www/PROJECTS"

	if (!base::dir.exists(home_dir)) { dir.create(home_dir,showWarnings = FALSE,recursive=T) }

	sessionsFile <<- paste0(home_dir,"/SHIESTA.projects")
	
	if(base::file.exists(sessionsFile)){
		if (base::file.size(sessionsFile)==0) {
			write.table(myDfSessions,file=sessionsFile, col.names=T,sep="\t",quote=F,row.names = F)
		}
		else {
			myDfSessions<-read.table(sessionsFile,sep="\t",header=T,stringsAsFactors = FALSE,row.names = NULL)
			json<-RJSONIO::toJSON(myDfSessions,.escapeEscapes = FALSE)
...
### End Reactive	
	...
	writeLog<-function(myText){
		if(projectLogFile != ""){
			...
			cat(paste0(format(Sys.time(), "%Y/%m/%d %H:%M:%S"),"\t",myText,"\n"), file = projectLog)
			close(projectLog)
		} else {
	...
	saveFileTable<-function(){
		if(projectFile != ""){
			write.table(myDfFiles$files,file=projectFile, col.names=T,sep="\t",quote=F,row.names = F)
		} else {
	...
	changeProjectDescription<-function(newData){
		newId<-newData[["id"]]
		if(any(myDfSessions$id==newData[["id"]])){
			toChange<-paste0(home_dir,"/",myDfSessions[myDfSessions$id==newData[["id"]],]$datapath)
			myDfSessions[myDfSessions$id==newData[["id"]],]$description<<-newData[["description"]]
			write.table(myDfSessions,file=sessionsFile, col.names=T,sep="\t",quote=F,row.names = F)
			writeLog(paste0("Project name changed to ",myDfSessions[myDfSessions$id==newData[["id"]],]$description))
			json<-RJSONIO::toJSON(myDfSessions,.escapeEscapes = FALSE)
			shinyjs::runjs(paste0("ProjectData['session']['description']=`",newData[["description"]],"`;updateProjectsList(`",json,"`)"))
		}
	}
...
### Project management functions
	...
	startNewProject<-function(ProjectData){
		#showNotification(paste0("Starting New Project: ",nrow(myDfSessions)+1))
		projectDir<<-paste0(home_dir,"/",ProjectData[["id"]])
		projectDir<<-paste0(home_dir,"/",ProjectData[["id"]])
		intializeProject(projectDir)
		...
		myDfSessions[nrow(myDfSessions)+1,] <<- c(
			 ProjectData[["id"]]
			,"STARTED"
			,format(Sys.time(), "%Y/%m/%d %H:%M:%S")
			,format(Sys.time(), "%Y/%m/%d %H:%M:%S")
			,paste0("",ProjectData[["id"]])
		...
		write.table(myDfSessions,file=sessionsFile, col.names=T,sep="\t",quote=F,row.names = F)
		outSessionText<-RJSONIO::toJSON(list(session=myDfSessions[myDfSessions$id==ProjectData[["id"]],]),.escapeEscapes = FALSE)
		outText<-RJSONIO::toJSON(list(files=myDfFiles$files),.escapeEscapes = FALSE)
...
#		checkProjectIntegrity()
	...
	openProject<-function(ProjectData){
		#showNotification(paste0("Opening project: ",ProjectData[["id"]]))
		projectDir<<-paste0(home_dir,"/",ProjectData[["id"]])
		myDfSessions[myDfSessions$id==ProjectData[["id"]],]$accessed <<- format(Sys.time(), "%Y/%m/%d %H:%M:%S")
		projectFile<<-paste0(projectDir,"/SHIESTA.files")
		...
		if(base::file.exists(projectFile)){
			currentFiles<-read.table(projectFile,sep="\t",header=T,stringsAsFactors = FALSE,row.names = NULL, quote="")
...
#		checkProjectIntegrity()
	...
	deleteProject<-function(ProjectData){
		...
		if(any(myDfSessions$id==ProjectData[["id"]])){
			toRemove<-paste0(home_dir,"/",myDfSessions[myDfSessions$id==ProjectData[["id"]],]$datapath)
			toRemoveDesc<-myDfSessions[myDfSessions$id==ProjectData[["id"]],]$description
			#showNotification(toRemove)
			if(dir.exists(toRemove)){
				dir_delete(toRemove)
				myDfSessions <<- myDfSessions[myDfSessions$id!=ProjectData[["id"]],]
				write.table(myDfSessions,file=sessionsFile, col.names=T,sep="\t",quote=F,row.names = F)
				json<-RJSONIO::toJSON(myDfSessions,.escapeEscapes = FALSE)
				#json<-jsonlite::toJSON(myDfSessions, force=TRUE)
				shinyjs::runjs(paste0("updateProjectsList(`",json,"`)"))
				#showNotification(paste0("Project ",toRemoveDesc," deleted"))
			}
			else {
				showNotification("DIR NOT EXISTS")
			}
		}
	}
	...
	loadGFFfeatures<- function(newData){
		...
		if (file.exists(gffFile)) {
			outText <- get_features(gffFile, includeCounts=TRUE, outFormat="JSON")
		}
		else {
			outText <- "new Object()"
		}
	...
	align_with_bowtie2<-function(newData) {
		...
		tryCatch({
			...
			if (grepl("\\.gz$",input1)) {
				...
				gunzip(input1, input1expanded, overwrite=FALSE, remove=FALSE)
				input1=input1expanded
			}
			if (grepl("\\.gz$",input2)) {
				input2expanded=paste0(projectDir,"/temp/",gsub("\\.gz$","",basename(newData$input2)))
				gunzip(input2, input2expanded, overwrite=FALSE, remove=FALSE)
...
if (!dir.exists("_GENOME_INDEX_PATH_FINAL_")) {
if (!dir.exists("_GENOME_INDEX_PATH_TEMP_")) { # REMOVE THIS LINE IN REPORT
	dir.create("_GENOME_INDEX_PATH_TEMP_")
}  # REMOVE THIS LINE IN REPORT
	print("* Indexing genome for the first time...")
	bowtie2_build(\'_GENOME_FILE_\', \'_GENOME_INDEX_TEMP_\', "--threads 6")
...
bowtie2(bt2Index=\'_GENOME_INDEX_FINAL_\', samOutput=\'_OUTPUT_SAM_\', seq1=\'_SEQ1_\', seq2=\'_SEQ2_\', \'_STRINGPARAMS_\')
')
			}
			else {
...
bowtie2(bt2Index=\'_GENOME_INDEX_FINAL_\', samOutput=\'_OUTPUT_SAM_\', seq1=\'_SEQ1_\', \'_STRINGPARAMS_\')
')
			}
			to_eval_template <- paste0(to_eval_template, 'asBam("_OUTPUT_SAM_", indexDestination=TRUE)\n')
			to_eval_template <- paste0(to_eval_template, 'file.remove("_OUTPUT_SAM_")\n')
			to_eval_template <- paste0(to_eval_template, 'param <- ScanBamParam(flag=scanBamFlag(isSecondaryAlignment=FALSE, isUnmappedQuery=FALSE),  what="seq") # REMOVE THIS LINE\n')
			to_eval_template <- paste0(to_eval_template, 'NALIGNED <<- countBam("_OUTPUT_BAM_", param=param)[["records"]] # REMOVE THIS LINE\n\n')
...
removeDupReads(input="_OUTPUT_BAM_", output="_OUTPUT_BAM_.tmp.bam", ',stringParamsRemoveDups,')
file.remove("_OUTPUT_BAM_")
file.remove("_OUTPUT_BAM_.bai")
file.rename("_OUTPUT_BAM_.tmp.bam", "_OUTPUT_BAM_")
indexBam("_OUTPUT_BAM_")
param <- ScanBamParam(flag=scanBamFlag(isSecondaryAlignment=FALSE, isUnmappedQuery=FALSE),  what="seq") # REMOVE THIS LINE
NODUPS <<- countBam("_OUTPUT_BAM_", param=param)[["records"]] # REMOVE THIS LINE IN REPORT\n
')
			}
...
if (file.exists("_SEQ1_.gz") && !file.exists("_SEQ1_")) { gunzip("_SEQ1_.gz", remove=FALSE) }
')
			if (newData$input2!=NOT_SELECTED) {
				to_eval_template <- paste0(to_eval_template,'if (file.exists("_SEQ2_.gz") && !file.exists("_SEQ2_")) { gunzip("_SEQ2_.gz", remove=FALSE) }
')
			}
			to_eval_template <- paste0(to_eval_template,'if (file.exists("_GENOME_FILE_.gz") && !file.exists("_GENOME_FILE_")) { gunzip("_GENOME_FILE_.gz", remove=FALSE) }
')
			...
			file_delete(paste0(output,".bai")) # we will index it later
			if (file_exists(input1expanded)) { file_delete(input1expanded) }
			if (file_exists(input2expanded)) { file_delete(input2expanded) }
...
if (!dir.exists("_GENOME_INDEX_PATH_FINAL_")) {
	dir.create("_GENOME_INDEX_PATH_TEMP_")
	print("* Indexing genome for the first time...")
	hisat2_build(references="_GENOME_FILE_", outdir="_GENOME_INDEX_PATH_TEMP_", prefix="_BASENAME_GENOME_", force=TRUE, p=6)
	file.rename("_GENOME_INDEX_PATH_TEMP_", "_GENOME_INDEX_PATH_FINAL_") # REMOVE THIS LINE IN REPORT
...
if (!file.exists("',out_final,'")) {
	...
	extract_splice_sites(features="_USE_GFF_", outfile="_TMP_GFF_")
	)  # REMOVE THIS LINE IN REPORT
...
hisat2(index=\'_GENOME_INDEX_FINAL_\', type=\'paired\', outfile=\'_OUTPUT_SAM_\', sequences=as.list(c(\'_INPUT1_\',\'_INPUT2_\')), _STRINGPARAMS_)
')
			}
			else {
...
hisat2(sequences=\'_INPUT1_\', index=\'_GENOME_INDEX_FINAL_\', type=\'single\', outfile=\'_OUTPUT_SAM_\',  _STRINGPARAMS_)
')
			}
			to_eval_template <- paste0(to_eval_template, 'asBam("_OUTPUT_SAM_", indexDestination=TRUE)\n')
			to_eval_template <- paste0(to_eval_template, 'file.remove("_OUTPUT_SAM_")\n')
			to_eval_template <- paste0(to_eval_template, 'param <- ScanBamParam(flag=scanBamFlag(isSecondaryAlignment=FALSE, isUnmappedQuery=FALSE),  what="seq") # REMOVE THIS LINE IN REPORT\n')
			to_eval_template <- paste0(to_eval_template, 'NALIGNED <<- countBam("_OUTPUT_BAM_", param=param)[["records"]] # REMOVE THIS LINE IN REPORT\n\n')
...
file.rename("_OUTPUT_BAM_.tmp.bam", "_OUTPUT_BAM_")
indexBam("_OUTPUT_BAM_")
param <- ScanBamParam(flag=scanBamFlag(isSecondaryAlignment=FALSE, isUnmappedQuery=FALSE),  what="seq") # REMOVE THIS LINE IN REPORT
NODUPS <<- countBam("_OUTPUT_BAM_", param=param)[["records"]] # REMOVE THIS LINE IN REPORT\n
')
			}
			...
			to_eval_template <- paste0(to_eval_template,'## ## 00005 Uncompress input files if necessary\nprint("* Uncompressing input files (if necessary)...")
if (file.exists("_INPUT1_.gz") && !file.exists("_INPUT1_")) { gunzip("_INPUT1_.gz", remove=FALSE) }
')
			if (newData$input2!=NOT_SELECTED) {
				to_eval_template <- paste0(to_eval_template,'if (file.exists("_INPUT2_.gz") && !file.exists("_INPUT2_")) { gunzip("_INPUT2_.gz", remove=FALSE) }
')
			}
			to_eval_template <- paste0(to_eval_template,'if (file.exists("_GENOME_FILE_.gz") && !file.exists("_GENOME_FILE_")) { gunzip("_GENOME_FILE_.gz", remove=FALSE) }
')
			...
			if (file.exists(use_gff)) {
				to_eval_template <- paste0(to_eval_template,'if (file.exists("',use_gff,'.gz") && !file.exists("',use_gff,'")) { gunzip("',use_gff,'.gz", remove=FALSE) }
')
			}
...
if (!dir.exists("_GENOME_INDEX_PATH_FINAL")) {
	dir.create("_GENOME_INDEX_PATH_TEMP")
	buildindex(basename="_GENOME_INDEX_TEMP_", reference="_GENOME_FILE_")
	file_move("_GENOME_INDEX_PATH_TEMP_", "_GENOME_INDEX_PATH_FINAL_")
}\n')
			if (file.exists(input2)) {
				to_eval_template=paste0(to_eval_template, 'align(index="_GENOME_INDEX_FINAL_", output_file="_OUTPUT_", readfile1="_INPUT1_", readfile2="_INPUT2_", _STRINGPARAMS_)\n')
			}
			else {
				to_eval_template=paste0(to_eval_template, 'align(index="_GENOME_INDEX_FINAL_", output_file="_OUTPUT_", readfile1="_INPUT1_", _STRINGPARAMS_)\n')
			}
...
removeDupReads(input="',theinput,'", output="',paste0(theoutput,".tmp"),'", ',stringParams,')
file_move("',paste0(theoutput,".tmp"),'", "',theoutput,'")
')
			eval(parse(text=to_eval))
...
#					nfb=nfb+1
#				}	
#				write.table(SAF,file=SAFFILE, row.names=FALSE, quote=FALSE, sep="\t")
			}
...
if (!file.exists("',SAFFILE,'")) {			
	...
	saf_from_gff("',GFFFILE,'", features=',feature_and_block,', outFile="',SAFFILE,'")
...
countsData<-featureCounts(annot.ext="',SAFFILE,'", isGTFAnnotationFile=FALSE, files="',theinput,'", isPairedEnd=',ISPAIRED,', ', stringParams,')
write.table(countsData$counts,file="',theoutput,'", append = FALSE, sep = "\t", col.names=F,row.names = TRUE, quote=FALSE)\n')
			eval(parse(text=to_eval))
			
			## add some lines to command (no to be evaluated)
			
			to_eval <- paste0(to_eval,'## ## 00005 Uncompress input files if necessary\nprint("* Uncompressing input files (if necessary)...")
if (file.exists("',GFFFILE,'.gz") && !file.exists("',GFFFILE,'")) { gunzip("',GFFFILE,'.gz", remove=FALSE) }
')			
...
')
			...
			if (file.exists(annots_file)) {
				if (grepl("\\.xlsx?$", annots_file)) {
					readTheTableCommand=paste0("ANNOTS_FILE_CONTENT <- read.xlsx('",annots_file,"')")
				}
				else {
					readTheTableCommand=paste0("ANNOTS_FILE_CONTENT <- read.table('",annots_file,"', sep='\t', header=TRUE, quote='', comment.char = '')")
				}
...
ANNOTS <- left_join(x=data.frame(ID=row.names(res)), y=ANNOTS_FILE_CONTENT, by=c('ID' = colnames(ANNOTS_FILE_CONTENT)[1]))
...
write.table(pdecoder, file='",projectDir,"/temp/",decoder_file,"', sep='\t', row.names=FALSE, quote=FALSE) # REMOVE THIS LINE
## ## 00350 Read raw counts from files
dds <-  DESeqDataSetFromHTSeqCount(sampleTable = decoder, directory = '",projectDir,"', design = ~ condition)
## ## 00375 Apply DESeq2 functions to detect differentially expressed features
dds <- DESeq(dds",stringParamsDESeq,")
res <- results(dds, contrast=c('condition', '",numerator_label,"', '",denominator_label,"')",stringParamsResults,")
...
DETAILS <- DETAILS %>% group_by(ID,Type,Strand) %>% summarise(Location=paste(Location,collapse=';')) %>% relocate(Location, .before = Strand)

## ## 00380 Create ID Type and Location columns
ID_Type_Location <- left_join(x=data.frame(ID=row.names(res)), y=DETAILS, by=c('ID' = 'ID'))
...
write.xlsx(file='",paste0(projectDir, '/temp/', output_file),"', final, showNA = TRUE)
")
...
')
			...
			if (file.exists(annots_file)) {
				if (grepl("\\.xlsx?$", annots_file)) {
					readTheTableCommand=paste0("ANNOTS_FILE_CONTENT <- read.xlsx('",annots_file,"', check.names=FALSE)")
				}
				else {
					readTheTableCommand=paste0("ANNOTS_FILE_CONTENT <- read.table('",annots_file,"', sep='\t', header=TRUE, quote='', comment.char = '', check.names=FALSE)")
				}
				code_to_add_annots=paste0("## ## 00383 Create additional annotation columns from ann file
",readTheTableCommand,"
## ## 00384 Remove problematic characters in annotations
...
# Merge annotations file with final gene ids.
ANNOTS <- left_join(x=data.frame(ID=row.names(res$table)), y=ANNOTS_FILE_CONTENT, by=c('ID' = colnames(ANNOTS_FILE_CONTENT)[1]))
...
d <- readDGE(fg, path = '",projectDir,"', labels = decoder$sample, header = FALSE)
d <- estimateCommonDisp(d)
d <- estimateTagwiseDisp(d)
et <- exactTest(d, pair = c('",denominator_label,"', '",numerator_label,"')",stringParams,")
res <- topTags(et, n = 100000000000, adjust.method = 'BH', sort.by = 'none', p.value = 100)
...
DETAILS <- DETAILS %>% group_by(ID,Type,Strand) %>% summarise(Location=paste(Location,collapse=';')) %>% relocate(Location, .before = Strand)
## ## 00380 Create ID Type and Location columns
ID_Type_Location <- left_join(x=data.frame(ID=row.names(res$table)), y=DETAILS, by=c('ID' = 'ID'))
",code_to_add_annots,"## ## 00385 Format differential expression columns
suppressWarnings( # REMOVE THIS LINE
...
")
	...
	create_FIESTA_from_DiffExpTable<-function(input, details) {
		...
		if (grepl("\\.xlsx?$", input)) {
			mydata <- read.xlsx(input, check.names=FALSE)
		}
		else {
			mydata <- read.table(input, sep="\t", header=TRUE, quote="", check.names=FALSE)
		}
		...
		write.table(rbind(colnames(mydata),mydata), fileConn, sep="\t", col.names=FALSE, row.names=FALSE, quote=FALSE, append=TRUE)
		close(fileConn)
	...
	initialize_this_file<-function(newData) {
		tryCatch({
			...
			file_move(newData$datapath,tmpProject)
...
#			}
			...
			nreads <- countFastq(tempPath)$records

			currentDesc <- paste0(format(nreads, big.mark=",", scientific=FALSE)," short reads")
			...
			prop["HIDDEN"]      = "NO"
			prop["MD5"]          = md5sum(tempPath)
			prop["CREATED"]      = format(Sys.time(), "%Y/%m/%d %H:%M:%S")
...
#			}
			...
			file_move(tempPath, newPath)
#			file_move(paste0(tempPath,".RQC.1.png"), paste0(newPath,".RQC.1.png"))
#			file_move(paste0(tempPath,".RQC.2.png"), paste0(newPath,".RQC.2.png"))
#			file_move(paste0(tempPath,".RQC.3.png"), paste0(newPath,".RQC.3.png"))
#			file_move(paste0(tempPath,".RQC.4.png"), paste0(newPath,".RQC.4.png"))

			if (file.exists(paste0(tempPath,".Raw"))) { file_move(paste0(tempPath,".Raw"), paste0(newPath,".Raw")) }
			if (file.exists(paste0(tempPath,".R"))) { file_move(paste0(tempPath,".R"), paste0(newPath,".R")) }

			file_move(paste0(tempPath,".props.json"), paste0(newPath,".props.json"))
	...
	initialize_BAM_file<-function(newData) {
		tryCatch({
			...
			if (grepl("\\.sam$",tempPath)) {
				to_eval=paste0('asBam("', tempPath,'", indexDestination=TRUE)')
				eval(parse(text=to_eval))
			...
			if (!file.exists(paste0(tempPath,".bai"))) {
				to_eval=paste0('indexBam("',tempPath,'")')
				eval(parse(text=to_eval))
			}
				
			#seqnames=names(scanBamHeader(tempPath)[[tempPath]]$targets)
			#param <- ScanBamParam(what=c('pos'), which=GRanges(seqnames, IRanges(1, 1e7)), flag=scanBamFlag(isUnmappedQuery=FALSE))
			
			ispaired=testPairedEndBam(tempPath,paste0(tempPath,".bai"))
			showConnections() # try to understand this
			...
			if (length(newData$dependencies)>0) {
				...
				if (ispaired) { 
					if (NODUPS > -1) { 
						...
						currentDesc=toString(paste0(format(NALIGNED, big.mark=",", scientific=FALSE)," paired-end primary alignments. ",format(NODUPS, big.mark=",", scientific=FALSE)," (",format(PERC, digits=1, big.mark=",", scientific=FALSE),"%) remain after dups. removal."))
						prop["ALIGNEDREADS"] = NODUPS
					}
					else {      		
						currentDesc=toString(paste0(format(NALIGNED, big.mark=",", scientific=FALSE)," paired-end primary alignments."))
					...
					if (NODUPS > -1) {
						PERC <- NODUPS/NALIGNED*100  
						currentDesc=toString(paste0(format(NALIGNED, big.mark=",", scientific=FALSE)," single-end primary alignments. ",format(NODUPS, big.mark=",", scientific=FALSE), " (",format(PERC, digits=1, big.mark=",", scientific=FALSE),"%) remain after dups. removal."))
						prop["ALIGNEDREADS"] = NODUPS
					}
					else {
						currentDesc=toString(paste0(format(NALIGNED, big.mark=",", scientific=FALSE)," single-end primary alignments."))
						prop["ALIGNEDREADS"] = NALIGNED
					}
				}
				NALIGNED <<- -1
				...
				param <- ScanBamParam(flag=scanBamFlag(isSecondaryAlignment=FALSE),  what="seq")
				naligned <- countBam(tempPath, param=param)[["records"]]
				param <- ScanBamParam(flag=scanBamFlag(isDuplicate=TRUE),  what="seq")
				ndups <-  countBam(tempPath, param=param)[["records"]]
				perc <- ndups/naligned*100
				if (ispaired) {          		
					currentDesc=paste0(format(naligned, big.mark=",", scientific=FALSE)," paired-end primary alignments. ", format(ndups, big.mark=",", scientific=FALSE)," (",format(perc, digits=1, big.mark=",", scientific=FALSE),"%) marked as duplicates (file provided by user)")
					prop["ALIGNEDREADS"] = naligned
					prop["DUPLICATEDREADS"] = ndups
				}
				else {
					currentDesc=paste0(format(naligned, big.mark=",", scientific=FALSE)," single-end primary alignments. ", format(ndups, big.mark=",", scientific=FALSE)," (",format(perc, digits=1, big.mark=",", scientific=FALSE),"%) marked as duplicates (",format(perc, digits=1, big.mark=",", scientific=FALSE),"%) (file provided by user)")
					prop["ALIGNEDREADS"] = naligned
					prop["DUPLICATEDREADS"] = ndups
				}
			}
...
#			MD5_BAI          = prop["MD5"]
			...
			file_move(tempPathBAI,newPathBAI)

			file_move(paste0(tempPath,".props.json"), paste0(newPath,".props.json"))

#			file_move(paste0(tempPath,".bai.props.json"), paste0(newPath,".bai.props.json"))
...
#			shinyjs::runjs(paste0("DataTable = ",outText,";renderFileTable('fcreated','down');resize();"))
	...
	initialize_FASTA_file<-function(newData) {
		tryCatch({
			...
			fai <- read.table(tempPathFAI,sep="\t")
			chrs=nrow(fai)
			if (chrs==1) {
				currentDesc=paste0(format(chrs, big.mark=",", scientific=FALSE)," chromosome: '",fai[1,1],"'")
			}
			else if (chrs==2) {
				currentDesc=paste0(format(chrs, big.mark=",", scientific=FALSE)," chromosomes: '", fai[1,1],"' and '", fai[2,1],"'")
			}
			else if (chrs==3) {
				currentDesc=paste0(format(chrs, big.mark=",", scientific=FALSE)," chromosomes: '", fai[1,1],"', '", fai[2,1], "' and '", fai[3,1],"'")
			}
			else {
				currentDesc=paste0(format(chrs, big.mark=",", scientific=FALSE)," chromosomes: '", fai[1,1],"', '", fai[2,1], "', '", fai[3,1], "'...")
			}
...
#			shinyjs::runjs(paste0("DataTable = ",outText,";renderFileTable('fcreated','down');resize();"))
			...
			file_move(tempPathFAI, newPathFAI)
#			file_move(paste0(tempPathFAI,".props.json"), paste0(newPathFAI,".props.json"))
	...
	initialize_GFF_file<-function(newData) {
		tryCatch({
			...
			if (grepl("\\.gtf$",tempPath)) { 
				track <- import(tempPath)
				export(track, con=paste0(gsub(".gtf$","",tempPath),".sorted.gtf"), format="gtf", index=TRUE)
			}
			else if (grepl("\\.gff3$",tempPath)) {
				track <- import(tempPath)
				export(track, con=paste0(gsub(".gff3$","",tempPath),".sorted.gff3"), format="gff3", index=TRUE)
			}			
			else if (grepl("\\.gff$",tempPath)) {
				track <- import(tempPath)
				export(track, con=paste0(gsub(".gff$","",tempPath),".sorted.gff"), format="gff3", index=TRUE)
			}			
	...
	create_accessory_files_for_GFF<-function(newData) {
		tryCatch({
			...
			plot_features(tempPath, outFile=paste0(tempPath,".png"), includeCounts=TRUE)
			saf_from_gff(tempPath, outFile=paste0(tempPath,".saf"), features="gene") # to generate .paths
			genes=length(readLines(paste0(tempPath,".saf")))-1
			try(suppressWarnings(extract_splice_sites(features=tempPath, outfile=paste0(tempPath,".introns"))))
			showConnections() # try to understand this
			...
			if (file.exists(paste0(tempPath,".introns")) && file.info(paste0(tempPath,".introns"))$size >0) {
				introns <- read.table(paste0(tempPath,".introns"), sep="\t", header=FALSE, quote="")
				...
				prop["DESCRIPTION"]=paste0(format(genes, big.mark=",", scientific=FALSE)," genes. ",format(prop["INTRONS"], big.mark=",", scientific=FALSE), " introns found (min: ",format(prop["MININTRONSIZE"])," nt; max: ",format(prop["MAXINTRONSIZE"], big.mark=",", scientific=FALSE), " nt; average: ", format(prop["AVGINTRONSIZE"], big.mark=",", scientific=FALSE)," nt; median: ", format(prop["MEDIANINTRONSIZE"], big.mark=",", scientific=FALSE)," nt)")
			}
				...
				prop["INTRONS"]=0
				prop["DESCRIPTION"]=paste0(format(genes, big.mark=",", scientific=FALSE)," genes. No introns found")
			}
			else {
				...
				prop["INTRONS"]=-1
				prop["DESCRIPTION"]=paste0(format(genes, big.mark=",", scientific=FALSE)," genes. Unable to analyze introns in this file")
			}
	...
	add_GFF_to_project<-function(newData) {
		tryCatch({
			...
			file_move(tempPathPAIRS,newPathPAIRS)
			file_move(tempPathPATHS,newPathPATHS)

			file_move(paste0(tempPath,".props.json"), paste0(newPath,".props.json"))
			file_move(tempPath,newPath)
			if (file.exists(paste0(tempPath,".png"))) { file_move(paste0(tempPath,".png"), paste0(newPath,".png")) }
			if (file.exists(paste0(tempPath,".Raw"))) { file_move(paste0(tempPath,".Raw"), paste0(newPath,".Raw")) }
			if (file.exists(paste0(tempPath,".R"))) { file_move(paste0(tempPath,".R"), paste0(newPath,".R")) }
			if (file.exists(paste0(tempPath,".introns"))) { file_move(paste0(tempPath,".introns"), paste0(newPath,".introns")) }
			if (file.exists(paste0(tempPathNoExtension,".sorted.gff3.bgz")))     { file_move(paste0(tempPathNoExtension,".sorted.gff3.bgz"),     paste0(newPathNoExtension,".sorted.gff3.bgz")) }
			if (file.exists(paste0(tempPathNoExtension,".sorted.gff3.bgz.tbi"))) { file_move(paste0(tempPathNoExtension,".sorted.gff3.bgz.tbi"), paste0(newPathNoExtension,".sorted.gff3.bgz.tbi")) }
			if (file.exists(paste0(tempPathNoExtension,".sorted.gtf.bgz")))      { file_move(paste0(tempPathNoExtension,".sorted.gtf.bgz"),      paste0(newPathNoExtension,".sorted.gtf.bgz")) }
			if (file.exists(paste0(tempPathNoExtension,".sorted.gtf.bgz.tbi")))  { file_move(paste0(tempPathNoExtension,".sorted.gtf.bgz.tbi"),  paste0(newPathNoExtension,".sorted.gtf.bgz.tbi")) }
			if (file.exists(paste0(tempPathNoExtension,".sorted.gff.bgz")))      { file_move(paste0(tempPathNoExtension,".sorted.gff.bgz"),      paste0(newPathNoExtension,".sorted.gff.bgz")) }
			if (file.exists(paste0(tempPathNoExtension,".sorted.gff.bgz.tbi")))  { file_move(paste0(tempPathNoExtension,".sorted.gff.bgz.tbi"),  paste0(newPathNoExtension,".sorted.gff.bgz.tbi")) }
	...
	initialize_COUNTS_file<-function(newData) {
		tryCatch({
			...
			tmpdata <- read.table(tempPath, sep="\t")
			nrows=nrow(tmpdata)
			...
			currentDesc=paste0(format(nrows, big.mark=",", scientific=FALSE)," features: ",first, currentDesc)

			propsFile    = paste0(tempPath,".props.json")

			prop <-list()
			prop["SAF"]          = basename(SAFFILE)
			prop["MD5SAF"]       = md5sum(SAFFILE)
			prop["NAME"]         = basename(tempPath)
	...
	add_COUNTS_to_project<-function(newData) {
		tryCatch({
			...
			if (!dir.exists(countsFolder)) { dir.create(countsFolder) }
	...
	add_DIFFEXP_to_project<-function(newData) {
		tryCatch({
			...
			if (file.exists(paste0(tempPath,".Raw"))) { file_move(paste0(tempPath,".Raw"), paste0(newPath,".Raw")) }
			if (file.exists(paste0(tempPath,".R"))) { file_move(paste0(tempPath,".R"), paste0(newPath,".R")) }
			#if (file.exists(gsub(".xlsx$", ".decoder.tsv", tempPath))) { file_move(gsub(".xlsx$", ".decoder.tsv", tempPath), gsub(".xlsx$", ".decoder.tsv", newPath)) }

			if (file.exists(paste0(tempPath,".FIESTA.html"))) { file_move(paste0(tempPath,".FIESTA.html"), paste0(newPath,".FIESTA.html")) }
	...
	add_ANN_to_project<-function(newData) {
		tryCatch({
			...
			if (grepl("\\.xlsx?$", tempPath)) {
				tmpdata <- read.xlsx(tempPath)
			}
			else {
				tmpdata <- read.table(tempPath,sep="\t", quote="", comment.char="", header=TRUE)
			}
			nrows = nrow(tmpdata)
			ncols = ncol(tmpdata)
			...
			currentDesc=paste0(format(nrows, big.mark=",", scientific=FALSE)," genes: ",firstrows," x ",ncols, " annotation fields: ",firstcols)
