	...
	for (var t=0;t<urls.length;t++) {
		if (urls[t]!==NOT_SELECTED) {
			...
			if (urls[t].match(/\.bam$/)) {
				...
				trackList[t].searchable=true;
				trackList[t].indexed=true;
				trackList[t].url="PROJECTS/"+CURRENT_PROJECT_ID+"/"+urls[t].replace(/.gff$/,"")+".sorted.gff.bgz";
				trackList[t].indexURL=trackList[t].url+".tbi";
			}
...
function open_browser(igvDiv, bamFiles, fastaFile, annotFile) {
	...
	if (annotFile!=="none") {
		...
		if (annotFile.match(/\.gtf$/)) {
			...
			options.reference.tracks.url=annotFile.replace(/.gtf$/,"")+".sorted.gtf.bgz";
			options.tracks[b].indexURL=annotFile.replace(/.gtf$/,"")+".sorted.gtf.bgz.tbi";
			options.reference.tracks.searchable=true;
		}
		else if(annotFile.match(/\.gff3$/)) {
			options.reference.tracks.indexed=true;
			options.reference.tracks.url=annotFile.replace(/.gff3$/,"")+".sorted.gff3.bgz";
			options.tracks[b].indexURL=annotFile.replace(/.gff3$/,"")+".sorted.gff3.bgz.tbi";
			options.reference.tracks.searchable=true;
		}
		else if(annotFile.match(/\.gff$/)) {
			options.reference.tracks.indexed=true;
			options.reference.tracks.url=annotFile.replace(/.gff$/,"")+".sorted.gff.bgz";
			options.tracks[b].indexURL=annotFile.replace(/.gff$/,"")+".sorted.gff.bgz.tbi";
			options.reference.tracks.searchable=true;
		}
	}
	if (bamFiles!=="none") {
