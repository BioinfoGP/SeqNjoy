	...
	for (var t=0;t<ALL_PROJECTS["status"].length;t++) {
		...
		<tr>
			...
			<td id="SessionDescription_Td_`+t+`" class="w100 DialogRow bgradient">
				`+ALL_PROJECTS["description"][t]+`
			</td>
			<td id="SessionStarted_Td" class="nowrap DialogRow bgradient">
				`+ALL_PROJECTS["started"][t]+`
			</td>
			<td class="nowrap DialogRow bgradient">
				<button class="buttonIcon" title="Delete project" onClick="deleteProject('`+ALL_PROJECTS["id"][t]+`')">
						...
						<table cellpadding=0 cellspacing=0 class="w100" border="0px">
							<tr>
								<td class="medipadded top" style="padding-top:1.2rem;">
									<div class="HomeLogo logo">SeqNjoy<br><div style="font-size:0.3em;">BioinfoGP CNB-CSIC 2022</div></div>
								</td>
...
User friendly dialog boxes and tools allow you to implement powerful statistical methods to your data and visually analyze the results without having to write lines of code (<a href="https://bioinfogp.cnb.csic.es/tools/seqnjoy/help" target="SeqNjoy_help">more info...</a>).
								...
								<td class="top medipadded w50" style="padding-left:1rem;">
									<div class="font15" style="padding-bottom:0.2em;padding-top:0.2em;">&nbsp;</div>
									<div class="justify">
				...
				<tr>
					<td>
						<div id="Home_Dialog" class="border padded shadow radius bwhite" style="margin-top:3rem;">
							<table id="Home_Dialog_Table" border="0px" cellpadding="0px" cellspacing="0px" class="w100 h100">
								<tr>
									<td id="Home_Dialog_Td_Up" class="DialogTitle" style="padding-bottom:1rem;">
										Available Projects
									</td>
								</tr>
								<tr>
									<td id="Home_Dialog_Td_Main" style="height:200px;">
										<div id="Sessions" class="autoscroll h100"><div style="width:100%;text-align:center;padding-top:5rem;">Loading available projects...</div>
									</td>
								</tr>
								<tr>
									<td id="Home_Dialog_Td_Bottom" style="padding-top:2rem;">
										<table id="NewSession_Table" cellspacing="0px" cellpadding="0px" border="0px" class="w100">
											<tr>
												<td id="NewSession_Td_Left" class="w100">
													<input type="textbox" id="NewSession_Input" class="w100 borderbox" name="NewSession_Input" placeholder="Enter a new project name" value="" oninput="this.value=safeString(this.value);" onkeyup="checkProjectName(trim(this.value));if(event.keyCode == 13) {_$('NewSession_Button').click();}"/> 
												</td>
												<td id="NewSession_Td_Right" class="w100">
													<button id="NewSession_Button" class="nowrap" name="NewSession_Button" disabled onclick="var id='P'+Date.now(); startNewProject(id,_$('NewSession_Input').value)">
														Start New Project
													</button>
												</td>
											</tr>
				...
				if(((showHiddenFiles && myObj["hidden"][i]=="YES") || (showAuxFiles && myObj["hidden"][i]=="AUXILIARY")|| myObj["hidden"][i]=="NO") && hideFileType[myObj["ftype"][i]]==false){
					...
					if (myObj["ftype"][i]=="bam" && !myObj["description"][i].match(/by user/)) {
						...
						for (var d=0;d<deps.length;d++) {
							if (deps[d].match(/\.f[an][^\.]*$/)) { var fastaFile="PROJECTS/"+CURRENT_PROJECT_ID+"/"+deps[d].replace(/&.+/,""); }
						}
						if (myObj["description"][i].match(/single-end/)) { var bamType="single"; } else { var bamType="paired"; }
						var bamFiles="PROJECTS/"+CURRENT_PROJECT_ID+"/"+myObj["datapath"][i]+"&type="+bamType;
						...
						var viewLink=`<button class="buttonIcon" style="background:`+color+`;" onclick="open_browser('igv-div','`+bamFiles+`','`+fastaFile+`','`+annotsFile+`');"><i class="fa fa-chart-area left buttonIconPadding" style="color:white;background:transparent;" onmouseover="this.style.color='black';" onmouseout="this.style.color='white';"></i></button>`;
					}
					else if (myObj["ftype"][i]=="fasta") {
						var annotsFile="none";
						var bamFiles="none";
						var fastaFile="PROJECTS/"+CURRENT_PROJECT_ID+"/"+myObj["datapath"][i];
						//htmlOut+=`<td class="bgradient nowrap center middle"  onclick="open_browser('igv-div','`+bamFiles+`','`+fastaFile+`','`+annotsFile+`');">`+myObj["ftype"][i]+`</td>`;
						var viewLink=`<button class="buttonIcon" style="background:`+color+`;" onclick="open_browser('igv-div','`+bamFiles+`','`+fastaFile+`','`+annotsFile+`');"><i class="fa fa-chart-area left buttonIconPadding" style="color:white;background:transparent;" onmouseover="this.style.color='black';" onmouseout="this.style.color='white';"></i></button>`;
					}
					else if (myObj["ftype"][i]=="gff") {
						var pngFile="PROJECTS/"+CURRENT_PROJECT_ID+"/"+myObj["datapath"][i]+".png";
						//htmlOut+=`<td class="bgradient nowrap center middle"  onclick="open_browser('igv-div','`+bamFiles+`','`+fastaFile+`','`+annotsFile+`');">`+myObj["ftype"][i]+`</td>`;
						var viewLink=`<button class="buttonIcon" style="color:white;background:`+color+`;" onclick="window.open('`+pngFile+`','gff','toolbar=no, menubar=no, resizable=yes');"><i class="fa fa-project-diagram left buttonIconPadding" style="font-size:0.8rem;background:transparent;" onmouseover="this.style.color='black';" onmouseout="this.style.color='white';"></i></button>`;
					}
					else if (myObj["ftype"][i]=="diffexp") {
						var htmlFile="PROJECTS/"+CURRENT_PROJECT_ID+"/"+myObj["datapath"][i]+".FIESTA.html";
						var viewLink=`<button class="buttonIcon" style="background:`+color+`;" onclick="window.open('`+htmlFile+`');"><i class="fa fa-table left buttonIconPadding" style="color:white;background:transparent;" onmouseover="this.style.color='black';" onmouseout="this.style.color='white';"></i></button>`;
					}
...
function checkProjectName(name){
	...
	for (var t=0;t<ALL_PROJECTS["description"].length;t++) {
		if (ALL_PROJECTS["description"][t]==name && name != ProjectData["session"]["description"]) {
			isNotDuplicated=false;
			break;
		}
	}

	if(name.match(/\w/) && isNotDuplicated){
